...
function PageHeader({ title, description, action, onAction }) { return <div className="workspace-page-header"><div><h1>{title}</h1><p>{description}</p></div>{action && <button className="primary-button" type="button" onClick={onAction}><Icon name="plus" size={16} />{action}</button>}</div>; }
...
function CreateCampaign({ data, setData, onNavigate }) {
  ...
  return <div className="workspace-page campaign-builder">
    ...
    <div className="campaign-builder-grid">
      <div className="campaign-builder-form-column">
      ...
      <div className="campaign-builder-preview-column">
        <section className="light-panel preview-panel sticky-preview"><div className="panel-heading"><div><span className="eyebrow">LIVE PREVIEW</span><h2>Creative preview</h2></div><span className="phase-chip">Responsive</span></div><Preview form={form} /></section>
      </div>
    </div>
