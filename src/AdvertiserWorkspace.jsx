...
const initialCreative = { creativeId: "", image: "", media: "", title: "", description: "", cta: "", destination: "", size: BANNER_SIZES[1], content: "", presentation: "Centered modal", video: "", thumbnail: "" };

function CreateCampaign({ data, setData, onNavigate, creatives = [] }) {
  ...
  const selectLibraryCreative = (id) => {
    const creative = creatives.find(item => item.id === id);
    setForm(previous => ({ ...previous, creativeId: id, image: format === "banner" ? creative?.name || "" : previous.image, media: ["native", "social"].includes(format) ? creative?.name || "" : previous.media, video: format === "video" ? creative?.name || "" : previous.video }));
    setErrors(previous => ({ ...previous, image: "", media: "", video: "" }));
  };
  const validate = () => {
    const next = {};
...
export default function AdvertiserWorkspace({ page, data, setData, onNavigate, creatives = [] }) {
  const content = useMemo(() => page, [page]);
  if (content === "campaigns") return <CampaignsPage data={data} onNavigate={onNavigate} />;
  if (content === "create-campaign") return <CreateCampaign data={data} setData={setData} onNavigate={onNavigate} creatives={creatives} />;
