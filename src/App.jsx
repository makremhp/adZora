...
export default function App() {
  ...
  const [publisherFinance, setPublisherFinance] = useState({ available: 125, pending: 32.5, earned: 642.5, withdrawn: 485, minimum: 50 });
  ...
  return <div className="app-shell">
    ...
    <main className="main-content">
       <header className="topbar"><button className="icon-button menu-button" onClick={() => setDrawerOpen(true)} aria-label="Open menu"><Icon name="menu" /></button><div className="breadcrumbs"><span>AdZora</span><Icon name="chevron" size={13} /><strong>{config.label}</strong><Icon name="chevron" size={13} /><span>{pageTitle}</span></div><div className="header-actions"><div className="header-balance-pair" aria-label="Advertiser and publisher balances"><div className="header-balance"><span>رصيد المعلن</span><strong>$0.00</strong></div><div className="header-balance"><span>رصيد الناشر</span><strong>$0.00</strong></div></div><button className="notification-button" aria-label="Notifications"><span className="notification-dot" /><Icon name="receipt" size={18} /></button><button className="avatar avatar-button" aria-label="Open account access" onClick={() => setAccountOpen(true)}>M</button></div></header>
