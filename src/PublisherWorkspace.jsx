...
function WebsitesPage({ data, setData, onNavigate, setNotice }) {
  ...
  const addWebsite = () => {
    ...
    try {
      ...
      if (parsedUrl.protocol !== "https:" || !parsedUrl.hostname || !parsedUrl.hostname.includes(".")) next.url = "Enter a valid website address, such as example.com.";
    } catch { next.url = "Enter a valid website address, such as example.com."; }
    setErrors(next); if (Object.keys(next).length) return;
    const website = { ...form, id: "website-" + Date.now(), name: form.name.trim(), url: new URL(normalizedUrl).toString(), zones: 0, impressions: 0, clicks: 0, revenue: "$0.00" };
