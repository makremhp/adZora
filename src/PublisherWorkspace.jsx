...
function WebsitesPage({ data, setData, onNavigate, setNotice }) {
  ...
  return <div className="workspace-page"><PageHeader title="My Websites" description="أضف Website Name وWebsite URL فقط. لا يختار الناشر نوع الإعلان أو الـCreative." action="Add Website" onAction={() => setShowForm(true)} />{showForm && <WebsiteForm form={form} setForm={setForm} errors={errors} onCancel={() => { setShowForm(false); setErrors({}); }} onSubmit={addWebsite} />}{!data.websites.length && !showForm ? <section className="light-panel"><EmptyState icon="globe" title="No websites yet" description="أضف موقعك للحصول على Universal AdZora Code." action="Add Website" onAction={() => setShowForm(true)} /></section> : data.websites.length > 0 && <section className="light-panel data-panel"><div className="panel-heading"><div><span className="eyebrow">PUBLISHER WEBSITES</span><h2>{data.websites.length} website{data.websites.length === 1 ? "" : "s"}</h2></div><span className="result-count">Universal code ready</span></div><div className="data-list">{data.websites.map(website => <div className="data-row publisher-website-row" key={website.id}><div className="row-main"><span className="row-icon"><MiniIcon name="globe" /></span><div><strong>{website.name}</strong><small>{website.url}</small></div></div><span className="status-badge pending">{website.status}</span><button className="universal-code-cta" type="button" onClick={() => onNavigate("ad-codes")}><MiniIcon name="code" size={18} /><span>Get Universal Code</span><MiniIcon name="arrow" size={17} /></button></div>)}</div></section>}</div>;
}

function AdCodesPage({ data, onNavigate, setNotice }) {
  const [copiedId, setCopiedId] = useState("");
  const universalCode = website => "<!-- AdZora Universal Code -->" + String.fromCharCode(10) + String.fromCharCode(60) + 'script src="https://cdn.adzora.com/universal.js" data-adzora-website="' + website.id + '" async' + String.fromCharCode(62) + String.fromCharCode(60) + "/script" + String.fromCharCode(62);
  const copyCode = async website => {
    const code = universalCode(website);
    try { await navigator.clipboard.writeText(code); setCopiedId(website.id); setNotice("Universal AdZora Code copied."); setTimeout(() => setCopiedId(""), 1800); } catch { setNotice("Copy is unavailable in this browser. Select the code manually."); }
  };
  return <div className="workspace-page"><PageHeader title="Universal AdZora Code" description="انسخ كودًا واحدًا إلى موقعك. لا تختار نوع الإعلان أو الـCreative؛ AdZora يتولى المطابقة والعرض تلقائيًا." action="Add Website" onAction={() => onNavigate("websites")} />{!data.websites.length ? <section className="light-panel"><EmptyState icon="code" title="Add a website first" description="بعد إضافة Website Name وWebsite URL سيظهر Universal AdZora Code هنا." action="Add Website" onAction={() => onNavigate("websites")} /></section> : <div className="code-list">{data.websites.map(website => <section className="light-panel code-card" key={website.id}><div className="code-card-heading"><div className="row-main"><span className="row-icon"><MiniIcon name="code" /></span><div><strong>{website.name}</strong><small>{website.url}</small></div></div><span className="status-badge ready">Universal</span></div><div className="code-note"><strong>One code for your website</strong><span>AdZora will choose an eligible advertiser Creative later. Publisher settings do not include ad type, size, or Creative.</span></div><pre className="code-box"><code>{universalCode(website)}</code></pre><div className="code-actions"><button className="primary-button" type="button" onClick={() => copyCode(website)}><MiniIcon name={copiedId === website.id ? "check" : "copy"} size={16} />{copiedId === website.id ? "Copied" : "Copy Universal Code"}</button><button className="ghost-button" type="button" onClick={() => onNavigate("websites")}><MiniIcon name="edit" size={16} />Manage website</button></div></section>)}</div>}</div>;
}
